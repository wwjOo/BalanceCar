...
#define PN2000_TO_DEG_P_S	0.061037f	/* GYR-ADC数据变化1 <==> 角速度变化0.061037f deg/s */
#define PN2000_TO_RAD_P_S	0.001065f	/* GYR-ADC数据变化1 <==> 角速度变化0.001065f rad/s */
#define PN8G_TO_CM_P_SS		0.2393f		/* ACC-ADC数据转化1 <==> 加速度变化0.2393f cm/s2*/



typedef struct
{
	float distance[XYZ]; 	/* 单位:cm */
	float gyr_rad_v[XYZ]; 	/* GYR角速度 */
	float gyr_rad_v_old[XYZ];
	float gyr_rad_a[XYZ];	/* GYR角加速度 */
	float centri_a[XYZ];	/* 向心加速度 */
